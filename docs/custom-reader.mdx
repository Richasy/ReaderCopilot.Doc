...
鉴于 PDF 使用场景的特殊性，有很多学术论文或者资料会以 PDF 的形式存储（而且通常很长，且费解）。故而我特别为 PDF 阅读器试验性地引入了类似 [ChatPDF](https://www.chatpdf.com) 的 AI 对话索引功能。

其基本原理是，将 PDF 内所有文本抽取出来，然后分段转换成向量，然后将你的问题也转换成向量后进行余弦相似比较，找出最匹配的上下文，将上下文与你的问题一起发送给 ChatGPT，最终得到一个严谨的回答。
